        ...
        $scope.editorFactory.promise.then(function () {
            ...
            $scope.editor.registerOnEvent(ORYX.CONFIG.EVENT_SELECTION_CHANGED, function (event) {
                ...
                if (shapes && shapes.length == 1) {
                	...
                	if(stencilId.indexOf("BPMNDiagram") == -1){
                		var title = shapes[0]._stencil._jsonStencil.title;//名称
                		
        	      		console.info("222_id:" + stencilId + ",name:" + title + ",selectedShape.resourceId:" + selectedShape.resourceId);
                	}
                	 */
            ...
            $scope.morphShape = function() {
            	...
            	*/
        		...
        		if (shapes && shapes.length > 0)
        		{
        			var resourceId =  shapes[0].resourceId;
    	      		...
    	      		var stencilItem = $scope.getStencilItemById(shapes.first().getStencil().idWithoutNs());
      	      		...
      	      		//console.info("resourceId:" + resourceId + ",stencilItem.id:" + stencilItem.id + ",stencilItem.name:" +  stencilItem.name);
      	      		var tmpUrl = 'editor-app/mypages/common_properties.jsp';
      	      		var dialogHeight=400;
      	      		var dialogWidth=550;
      	      		var title='';
      	      		if(stencilItem.id == "StartNoneEvent"){//开始节点
      	      			title = '开始节点属性';
      	      			tmpUrl = 'editor-app/mypages/start_properties.jsp';
      	      			dialogHeight=350;
      	      			dialogWidth=500;
      	      		}else if(stencilItem.id == "EndNoneEvent"){//结束节点
      	      			title = '结束节点属性';
      	      			tmpUrl = 'editor-app/mypages/end_properties.jsp';
      	      			dialogHeight=350;
      	      			dialogWidth=500;
      	      		}else if(stencilItem.id == "UserTask"){//用户活动
      	      			title = '用户活动节点属性';
      	      			tmpUrl = 'editor-app/mypages/userTask_properties.jsp';
      	      			dialogHeight=420;//375
      	      			dialogWidth=600;
      	      		}else if(stencilItem.id == "ServiceTask"){//服务活动
      	      			title = '服务活动节点属性';
      	      			tmpUrl = 'editor-app/mypages/service_properties.jsp';
      	      			dialogHeight=400;
      	      			dialogWidth=550;
      	      		}else if(stencilItem.id == "SequenceFlow"){
	      	      		title = '顺序流节点属性';
	  	      			tmpUrl = 'editor-app/mypages/line_properties.jsp';
	  	      			dialogHeight=420;
	  	      			dialogWidth=600;
      	      		}else{
      	      			return ;
      	      		}
      	      		tmpUrl = tmpUrl +"?modelId="+_MODEL_ID+'&resourceId='+resourceId ;
      	      		var toTop=(window.screen.availHeight -dialogHeight)/2-30;//计算模态窗口距顶部的距离，用于上下居中
      	      		var toLeft=(window.screen.availWidth  -dialogWidth)/2;//计算模态窗口距顶部的距离，用于左右居中
	      	      	var opt = {
	        				title:title,
	        				width:dialogWidth,
	        				height:dialogHeight,
	        		...
	        		jQuery.dlg(tmpUrl, opt);
      	      		//window.open(tmpUrl, title, "toolbar=no, location=yes, directories=no, status=no, menubar=no, scrollbars=yes, resizable=no, copyhistory=yes,top="+toTop+",left="+toLeft+",width="+dialogWidth+",height="+dialogHeight);
        		}
            	...
            	jQuery('#test').window('refresh', '/flow_design/editor-app/mypages/common_properties2.html');
            	*/
            	
            	/*
            	   Custombox.open({
                       //target:     'editor-app/custombox/demo/xhr/ajax.html',
            		   target:     'editor-app/mypages/common_properties2.html',
                       effect:     'door',
            ...
            $scope.dbc = function() {
            	...
            	if(shapes == ""){
	            	// var jsonStr = $scope.editor.getJSON();
	            	 //console.info("jsonStr:" + JSON.stringify(jsonStr));
	            	  var modelMetaData = $scope.editor.getModelMetaData();
//	            	  console.info("id:" + modelMetaData.modelId);
	            	 // console.info("modelMetaData:" + JSON.stringify(modelMetaData));
	            	 
	            	if (_MODEL_ID)
	         		{
	            		var toTop=(window.screen.availHeight -400)/2-30;//计算模态窗口距顶部的距离，用于上下居中
		        		var toLeft=(window.screen.availWidth  -550)/2;//计算模态窗口距顶部的距离，用于左右居中
	            		...
	            		jQuery.dlg(URL, opt);
	         			//window.open(URL,"全局属性","width=550,height=400,toolbar=no, location=yes, directories=no, status=no, menubar=no, scrollbars=yes, resizable=no, copyhistory=yes,top="+toTop+",left="+toLeft);
	         		}
            	}
            ...
            $scope.quickAddItem = function(newItemId) {
            	$scope.safeApply(function () {
            		...
            		if (shapes && shapes.length == 1)
            		{
	        			...
	        			jQuery.ajax({
	        				type:'post',
	        				url:ACTIVITI.CONFIG.context + "/initNodeProperties",
	        				data:{
	        					resourceId:command.shape.resourceId,
        						modelId:_MODEL_ID,
        						ctype:newItemId,
	        				},
	        				dataType:'text',
	        				success:function(data){
	        					/*初始化节点完毕后紧接着初始化连接关系 start*/
	        					var ctype = targetStencil.id();
	        					ctype = ctype.substr(ctype.indexOf("#") + 1);
	        					jQuery.ajax({
	    	        				type:'post',
	    	        				url:ACTIVITI.CONFIG.context + "/initNodeProperties",
	    	        				data:{
	    	        					resourceId:command.edge.resourceId,
	            						modelId:_MODEL_ID,
	            						ctype:ctype,
	            						fromResourceId:option.connectedShape.resourceId,
	            						toResourceId:command.shape.resourceId,
	    	        				},
	    	        				dataType:'text',
	    	        				success:function(data){
	    	        					console.log("开始节点："+ option.connectedShape.resourceId);//开始节点
	    	    	        			console.log("连接线："+ command.edge.resourceId);//线
	    	    	        			console.log("结束节点："+ command.shape.resourceId);//结束节点
	    	        					console.log("quick menu add");
	    	        				}
	    	        			})
	    	        			/*初始化节点完毕后紧接着初始化连接关系 end*/
	        				}
	        			})
	        			/* 通过快捷菜单直接添加节点 end*/
            		}
            ...
            if ($scope.dragCanContain) {
            	...
            	if ($scope.quickMenu)
            	{
            		...
            		if (shapes && shapes.length == 1)
            		{
	        			...
	        			/* 通过快捷菜单中的SequenceFlow连接节点触发事件 start*/
	        			if(containedStencil.idWithoutNs() == 'SequenceFlow'){
		        			jQuery.ajax({
		        				...
		        				data:{
		        					resourceId:command.shape.resourceId,
	        						modelId:_MODEL_ID,
	        						ctype:containedStencil.idWithoutNs(),
	        						fromResourceId:command.connectedShape.resourceId,
	        						toResourceId:command.currentReference.resourceId,
		        				},
		        				dataType:'text',
		        				success:function(data){
		        					console.log("开始节点"+ command.connectedShape.resourceId);//开始节点
				        			console.log("连接线"+ command.shape.resourceId);//线
				        			console.log("结束节点"+ command.currentReference.resourceId);//结束节点
		        					console.info("quick menu connecte");
		        				}
		        			})
	        			}
	        			/* 通过快捷菜单中的SequenceFlow连接节点触发事件 end*/
            		}
            	}
            	else
            	{
                    ...
                    jQuery.ajax({
                    	...
                    	dataType:'text',
                    	success:function(data){
                    		console.info("初始化"+item.id+":"+command.shape.resourceId);
                    	}
                    })
                    /*从界面左侧节点菜单拖拽节点到画布触发事件 end*/
        ...
        $scope.dragCallback = function (event, ui) {
        	//console.info("dragCallback=================:" + $scope.draggedElement);
            if ($scope.dragModeOver != false) {
            	//console.info("dragModeOver=================")
        ...
        var saveFlowJson = function(newValue, oldValue, scope){
        ...
        $scope.$watch($scope.getFlowJson, saveFlowJson);
        */
