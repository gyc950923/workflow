...
//    },
    ...
    editDelete: function(){
        ...
        //console.info("selection_edit:" + selection[0].resourceId);
	...
	dockerMovedFinished: function(event) {
		...
		if(this.outerDockerNotMoved) {
			...
			if (this.docker.parent){
				...
				/*连接线重新指向新目标节点时触发事件 start*/
				if(command.shape.getStencil().idWithoutNs() == "SequenceFlow"){
					var fromResourceId = "";
					var toResourceId = ""
					if(command.shape.incoming.length > 0){
						fromResourceId = command.shape.incoming[0].resourceId;
					}
					if(command.shape.outgoing.length > 0){
						toResourceId = command.shape.outgoing[0].resourceId;
					}
					jQuery.ajax({
        				type:'post',
        				...
        				success:function(data){
        					console.log("开始节点："+ fromResourceId);//开始节点
    	        			console.log("连接线："+ command.shape.resourceId);//线
    	        			console.log("结束节点："+ toResourceId);//结束节点
        					console.log("---dockerMovedFinished---");
        				}
        			})
				}
				/*连接线重新指向新目标节点时触发事件 end*/
			}
		}
