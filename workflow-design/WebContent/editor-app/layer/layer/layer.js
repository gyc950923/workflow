...
Class.pt.creat = function(){
  ...
  jQuery(document).off('keydown', ready.enter).on('keydown', ready.enter);
  that.layero.on('keydown', function(e){
    jQuery(document).off('keydown', ready.enter);
  });
...
Class.pt.move = function(){
  ...
  jQuery(config.move).on('mousedown', function(M){  
    M.preventDefault();
    if(jQuery(this).attr('move') === 'ok'){
      conf.ismove = true;
      conf.layero = jQuery(this).parents('.'+ doms[0]);
      var xx = conf.layero.offset().left, yy = conf.layero.offset().top, ww = conf.layero.outerWidth() - 6, hh = conf.layero.outerHeight() - 6;
      if(!jQuery('#layui-layer-moves')[0]){
        jQuery('body').append('<div id="layui-layer-moves" class="layui-layer-moves" style="left:'+ xx +'px; top:'+ yy +'px; width:'+ ww +'px; height:'+ hh +'px; z-index:2147483584"></div>');
      }
      conf.move = jQuery('#layui-layer-moves');
...
Class.pt.callback = function(){
  ...
  if(config.shadeClose){
    jQuery('#layui-layer-shade'+ that.index).on('click', function(){
      layer.close(that.index);
    });
  } 
...
ready.reselect = function(){
  $.each(jQuery('select'), function(index , value){
...
layer.style = function(index, options){
  var layero = jQuery('#'+ doms[0] + index), type = layero.attr('type');
  var titHeight = layero.find(doms[1]).outerHeight() || 0;
  var btnHeight = layero.find('.'+doms[6]).outerHeight() || 0;
...
layer.restore = function(index){
  var layero = jQuery('#'+ doms[0] + index), area = layero.attr('area').split(',');
...
layer.full = function(index){
  var layero = jQuery('#'+ doms[0] + index), timer;
  ready.record(layero);
...
layer.title = function(name, index){
  var title = jQuery('#'+ doms[0] + (index||layer.index)).find(doms[1]);
  title.html(name);
};
...
layer.close = function(index){
  ...
  jQuery('#layui-layer-moves, #layui-layer-shade' + index).remove();
  ...
  loadImage(data[start].src, function(img){
    ...
    dict.index = layer.open($.extend({
      ...
      area: function(){
         ...
         var winarea = [jQuery(window).width() - 50, jQuery(window).height() - 50];
