        ...
        function addBlocks(div, data, cellWidth, start) {
            ...
            for (var i = 0; i < data.length; i++) {
                for (var j = 0; j < data[i].series.length; j++) {
                    ...
                    var status = "";
                    if(undefined != data[i].series[j]["sjstartyear"]){
                    	var s = data[i].series[j]["sfzjzxm"]+data[i].series[j]["status"];
                    	if(undefined != jsonStatus[s])
                    		status = "状态:"+jsonStatus[s]+"      ";
                    }
                    	
                    
                    //文本框显示天数
                    block.append(jQuery("<div>", { "class": "ganttview-block-text" }).text(status+"用时:"+size+"天"));
	...
	var Behavior = function (div, opts) {
		...
		function apply() {
            ...
            if (opts.behavior.resizable) { 
            	bindBlockResize(div, opts.cellWidth, opts.start, opts.behavior.onResize,opts.readOnly); 
        	}
            
            if (opts.behavior.draggable) { 
            	bindBlockDrag(div, opts.cellWidth, opts.start, opts.behavior.onDrag,opts.readOnly); 
        	}
		}
        ...
        function bindBlockResize(div, cellWidth, startDate, callback, readOnly) {
        ...
        function bindBlockDrag(div, cellWidth, startDate, callback, readOnly) {
        	jQuery("div.ganttview-block", div).draggable({
        		disabled: readOnly,//lils  新增属性  进度条禁止拖动
        		axis: "x", 
